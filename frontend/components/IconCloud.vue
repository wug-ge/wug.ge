...
const props = defineProps<{ images: any, ariaLabel: any}>();
  ...
  function animate() {
    ...
    imagePositions.value.forEach((icon, index) => {
      ...
      if (imageCanvasesRef.value[index] && imagesLoadedRef.value[index]) {
        ...
        let imageName = images[index].split("/")[images[index].split("/").length - 1];
        imageName = imageName.replaceAll('dot', '.');
        ctx.fillText(imageName, 0, 30);
      }
      ctx.restore();
    });
